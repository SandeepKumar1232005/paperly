          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in duration-200`}>
              <div className={`max-w-[85%] flex flex-col gap-1.5 ${isMe ? 'items-end' : 'items-start'}`}>
                <div 
                  className={`px-4 py-2.5 rounded-2xl text-sm shadow-sm relative group/msg transition-all ${
                    isMe 
                ...
                >
                  ...
                  {msg.replyTo && (
                    <div className={`mb-2 p-2 rounded-lg text-[11px] border-l-4 ${
                      isMe ? 'bg-indigo-700/50 border-white/40 text-indigo-100' : 'bg-slate-100 border-indigo-400 text-slate-500'
                    }`}>
